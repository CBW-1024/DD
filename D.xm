...
//  DDAdBlock.xm
//  插件名: DD广告拦截   版本: 1.0.3
//  说明: 参考 D.txt 重建为独立单文件 tweak。D.txt 经实机验证"公众号广告拦截完美、
//        小程序中间无广告"，有效性来源于三层：① 原生广告数据/解析层
//        (WCAdvertiseDataHelper / BrandTL* / BrandAdDataParser / WCFinderComment /
//        WCAdFinderInfo 等)；② WebView URL 黑名单 + document-start 注入 JS +
//        DOM sweep（公众号 MMWebViewController、小程序 WAWebViewController）；
//        ③ 广告上报/推送抑制。本版完整保留这三层，不删除 D.txt 任何已验证
//        有效的 Hook；视频号评论区严格沿用 D.txt 写法（仅 neutralize 广告字段，
//        不 return nil、不新增未验证类名）。
//  分组: 广告拦截场景 = 朋友圈/公众号/视频号/直播/搜索(直播下)/小程序
//        进阶拦截 = 激励广告快速跳过（仅此项；无青少年弹窗、无网络层开关、无实验开关）
//  开关: 8 个，默认全关；显式 setter + synchronize，关闭立即持久化、不自动回弹；不使用宏。
//
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        if ([ud objectForKey:kDDAdBlockMasterKey] == nil) {
            [ud setBool:NO forKey:kDDAdBlockMasterKey];
        }
        if ([ud objectForKey:kDDAdBlockMomentsKey] == nil) {
            [ud setBool:NO forKey:kDDAdBlockMomentsKey];
        }
        if ([ud objectForKey:kDDAdBlockBrandKey] == nil) {
            [ud setBool:NO forKey:kDDAdBlockBrandKey];
        }
        if ([ud objectForKey:kDDAdBlockFinderKey] == nil) {
            [ud setBool:NO forKey:kDDAdBlockFinderKey];
        }
        if ([ud objectForKey:kDDAdBlockLiveKey] == nil) {
            [ud setBool:NO forKey:kDDAdBlockLiveKey];
        }
        if ([ud objectForKey:kDDAdBlockMiniProgramKey] == nil) {
            [ud setBool:NO forKey:kDDAdBlockMiniProgramKey];
        }
        if ([ud objectForKey:kDDAdBlockSearchKey] == nil) {
            [ud setBool:NO forKey:kDDAdBlockSearchKey];
        }
        if ([ud objectForKey:kDDAdBlockRewardedFastPassKey] == nil) {
            [ud setBool:NO forKey:kDDAdBlockRewardedFastPassKey];
        }
        [ud synchronize];
...
- (void)saveAdPullCompareInfo:(id)arg1 {
    if (ddActive() && [DDAdBlockConfig sharedConfig].moments) {
        return;
    }
    %orig;
}
...
// URL 黑名单（覆盖公众号文章内广告 iframe / 素材 / 小程序广告素材 / 联盟广告）
...
// ---------- 3. 视频号广告拦截（严格沿用 D.txt：仅 neutralize 广告字段，不 return nil） ----------
...
- (void)setupSubviews {
    if (ddActive() && [DDAdBlockConfig sharedConfig].live) {
        return;
    }
    %orig;
}
...
- (BOOL)isValid {
    if (ddActive() && [DDAdBlockConfig sharedConfig].search) {
        return NO;
    }
    return %orig;
}
